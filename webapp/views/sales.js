/* Flipcheck Web App — Verkaufs-Tracker (ported from v2) */
const SalesView = (() => {
  let _el       = null;
  let _chart    = null;
  let _chartDnt = null;
  ...
  const _collator     = new Intl.Collator("de-DE");
  ...
  function exportCSV(sold) {
    ...
    sold.forEach(i => {
      ...
      const _feeAmt = _calcMarketFee(_vk2, i.market || "ebay", i.cat_id);
  ...
  function buildHTML(filtered) {
    ...
    <div class="page-header">
      <div class="page-header-left">
        <h1>Verkäufe</h1>
        <p>Deine abgeschlossenen Flips im Überblick</p>
    ...
    <div class="sales-kpi-grid mb-16">
      ...
      <div class="comp-kpi-tile">
        ...
        <div class="comp-kpi-lbl">Umsatz</div>
      </div>
    ...
    <div class="sales-charts-row mb-16">
      ...
      <div class="panel sales-chart-side">
        <div class="kpi-label mb-12">Plattform-Verteilung</div>
        ${Object.keys(pSplit).length > 0 ? `
          <div class="chart-wrap" style="height:140px">
            <canvas id="salesChartDnt"></canvas>
          </div>
        ` : `<div class="sales-empty-mini">Keine Daten vorhanden</div>`}
      </div>
    </div>
    ...
    <div class="sales-bottom-row mb-16">
      <div class="panel">
        <div class="kpi-label mb-12">Top 5 Flips</div>
        ${top5.length > 0 ? `
    ...
    <div class="panel">
      ...
      ${filtered.length > 0 ? buildTable(filtered) : `<div class="sales-empty-mini">Keine Ergebnisse für diesen Filter</div>`}
    </div>
    `;
  }
    ...
    <table class="data-table sales-table">
      ...
      <tbody>
        ${rows.map(i => {
          ...
          const _feeAmt = _calcMarketFee(_vk, i.market || "ebay", i.cat_id);
  ...
  function buildEmpty() {
    ...
    <div class="sales-empty">
      ...
      <div class="sales-empty-sub">Markiere Artikel im Inventar als verkauft, um sie hier zu sehen.</div>
      <button class="btn btn-primary mt-12" id="btnGoInventory">Zum Inventar</button>
    </div>`;
  }
  ...
  function attachEvents(filtered) {
    ...
    if (btnGoInv) btnGoInv.addEventListener("click", () => {
      if (typeof App !== "undefined" && typeof App.navigateTo === "function") App.navigateTo("inventory");
    });
    ...
    _el.addEventListener("click", async e => {
      ...
      const confirmed = confirm(`Verkauf rückgängig machen?\n„${name}" wird zurück ins Inventar verschoben.`);
      if (!confirmed) return;
      ...
      try {
        ...
        if (typeof Toast !== "undefined") Toast.success("Verkauf rückgängig", `„${name}" ist wieder im Inventar.`);
        render();
      } catch {
        if (typeof Toast !== "undefined") Toast.error("Fehler", "Verkauf konnte nicht rückgängig gemacht werden.");
      }
    });
  }
  ...
  function initCharts(filtered) {
    ...
    // Read CSS design tokens at runtime
