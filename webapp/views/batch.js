/* Flipcheck Web App — Batch Flipcheck View */
const BatchView = (() => {
  let _container = null;
  let _results   = [];
  let _running   = false;
  ...
  // ─── eBay DE category fee structure ───────────────────────────────────────
  const CATEGORIES = [
    ...
    return `
      <div class="page-header">
        <div class="page-header-left">
          ...
          <p>Mehrere EANs gleichzeitig prüfen</p>
        </div>
        <div class="page-header-actions" id="batchHeaderActions">
          ${isVat
            ? `<div class="fc-info-pill fc-info-pill--accent">
                 ...
                 USt. 19% aktiv — Gebühren werden netto berechnet
               </div>`
            : ""
          }
        </div>
      ...
      <div class="fc-split-380">
        ...
        <div class="panel" id="batchInputPanel">
          <h3 class="panel-title">Batch-Eingabe</h3>
          ...
          <div class="row" style="margin:12px 0;gap:8px;align-items:center">
            ...
            <span class="text-xs text-muted">oder manuell eingeben</span>
            <div style="flex:1;height:1px;background:var(--border)"></div>
          </div>
          ...
          <div class="input-group" style="margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
              <label class="input-label" style="margin:0">EAN-Liste</label>
            </div>
            <textarea id="batchEanList" class="textarea" rows="5" placeholder="4010355360205&#10;4006381333955&#10;4250538302777…"></textarea>
          </div>

          <div class="input-group" style="margin-bottom:12px">
            <label class="input-label">Standard-EK${isVat ? " <span style='color:var(--accent);font-weight:500'>(brutto)</span>" : " (€)"}</label>
            <div class="input-prefix-wrap">
              <span class="prefix">€</span>
              <input id="batchDefaultEk" class="input" type="number" step="0.01" min="0.01" placeholder="z.B. 18.50" />
            </div>
            <span class="input-hint">${isVat ? "Brutto-EK wird automatisch in Netto umgerechnet" : "Wird für alle EANs ohne eigenen EK verwendet"}</span>
          </div>
          ...
          <div class="input-group" style="margin-bottom:12px">
            ...
            <span class="input-hint">Bestimmt die Verkaufsprovision</span>
          </div>
          ...
          <div id="batchProgress" style="display:none" class="panel panel-sm mb-16">
            <div class="batch-progress">
              <div class="batch-progress-row">
                <span class="batch-progress-label" id="batchProgressLabel">Wird verarbeitet…</span>
  ...
  function renderResultsEmpty() {
    return `
      <div class="empty-state" style="padding:60px">
        ...
        <p class="empty-sub">EANs eingeben oder CSV hochladen und Batch starten</p>
      </div>
    `;
  }
    ...
    return `
      <div class="batch-summary-bar" id="batchSummaryPanel">
        ...
        <div class="batch-sum-kpi">
          ...
          <span class="batch-sum-label">Gesamt-Profit</span>
        </div>
        <div class="batch-sum-sep"></div>
        <div class="batch-sum-kpi">
          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th class="col-right">EK</th><th class="col-right">\u2300 VK</th>
                <th class="col-right">Profit</th><th class="col-right">Marge</th>
                <th class="col-right">Verk. 30T</th><th>Verdikt</th><th></th>
              </tr>
            </thead>
  ...
  function parseFile(file, container) {
    ...
    reader.onload = e => {
      ...
      if (eans.length === 0) {
        Toast.warning("Keine EANs gefunden", "Die Datei enthält keine gültigen EAN-Codes.");
        return;
      }

      const textarea = container.querySelector("#batchEanList");
      if (textarea) textarea.value = eans.join("\n");
      Toast.info("CSV importiert", `${eans.length} EANs aus Datei geladen`);
    };
  ...
  async function runBatch(container) {
    ...
    if (uniqueEans.length === 0) {
      Toast.warning("Keine EANs", "Bitte gib mindestens eine gültige EAN ein.");
      return;
    }
    const dupCount = rawEans.length - uniqueEans.length;
    if (dupCount > 0) {
      Toast.info("Duplikate entfernt", `${dupCount} doppelte EANs wurden entfernt.`);
    }

    const defaultEk = parseFloat(defaultEkInput?.value);
    if (!defaultEk || defaultEk <= 0) {
      Toast.error("EK fehlt", "Bitte einen Standard-EK eingeben.");
    ...
    if (progressLabel) progressLabel.textContent = "Fertig!";
    ...
    const doneMsg   = `${uniqueEans.length} EANs gepr\u00fcft \u00b7 ${buyCount} BUY${errCount > 0 ? ` \u00b7 ${errCount} Fehler` : ""}`;
    Toast.success("Batch abgeschlossen", doneMsg);
  ...
  function exportCsv() {
    ...
    Toast.success("CSV exportiert", "Batch-Ergebnisse wurden als CSV heruntergeladen.");
  }

  // ─── Add BUYs to Inventory ─────────────────────────────────────────────────
  async function addBuysToInventory() {
    const buys = _results.filter(r => r.verdict === "BUY" && !r.error);
    if (!buys.length) { Toast.info("Keine BUYs", "Es gibt keine BUY-Ergebnisse zum Hinzuf\u00fcgen."); return; }
    ...
    Toast.success("Zum Inventar hinzugef\u00fcgt", `${added} BUY-Artikel wurden ins Inventar \u00fcbernommen.`);
  }
