/* Flipcheck Web Dashboard — Marktplatz-Vergleich (live: eBay + Kaufland + Amazon via Keepa) */

// Ensure API.compare exists (web dashboard)
if (!API.compare) {
  API.compare = (ean, ek) => API.call("/compare", { method: "POST", body: JSON.stringify({ ean, ek }) });
}
...
const MarketplaceView = (() => {
    ...
    return `
      <div class="page-header">
        <div class="page-header-left">
          <h1>Marktplatz-Vergleich</h1>
          <p>Vergleiche Preise & Gewinn auf eBay, Kaufland und Amazon</p>
      ...
      <div id="mpEmpty" class="empty-state" style="margin-top:48px">
        ...
        <p class="empty-title">Kein Vergleich aktiv</p>
        <p class="empty-sub">Gib eine EAN oder ASIN ein, um Preise auf allen Marktplätzen zu vergleichen.</p>
      </div>
  ...
  function updateStatsBar() {
    ...
    bar.innerHTML = `
      ...
      ${asin ? `<span class="sales-stat-pill" style="font-family:monospace;font-size:10px">ASIN: ${_esc(asin)}</span>` : ""}
      ${ean  ? `<span class="sales-stat-pill" style="font-family:monospace;font-size:10px">EAN: ${_esc(ean)}</span>` : ""}
    `;
  }
  ...
  async function doSearch() {
    ...
    if (!rawInput) {
      Toast.error("EAN fehlt", "Bitte EAN oder ASIN eingeben.");
      return;
    }
    ...
    if (asinDetected) {
      ...
      try {
        const adata = await API.amazonCheck(rawInput.toUpperCase(), null, 0, "mid", "fba");
        if (adata?.ean) {
          ean = adata.ean;
          ...
          Toast.error("Keine EAN gefunden", `ASIN ${rawInput.toUpperCase()} hat keine EAN. Amazon-Karte zeigt ASIN-Daten.`);
          btn.disabled = false;
    ...
    btn.innerHTML = `<span class="mp-spinner"></span> Laden…`;

    let data;
    try {
      data = await API.compare(ean, ek);
    } catch (e) {
  ...
  function renderProduct(d) {
    ...
    _el.querySelector("#mpProduct").innerHTML = `
      <div class="mp-product-inner">
        ${d.image_url
          ? `<img class="mp-prod-img" src="${_esc(d.image_url)}" alt="" loading="lazy"/>`
        ...
        <div class="mp-prod-info">
          <div class="mp-prod-title">${_esc(title.slice(0, 90))}</div>
          <div class="mp-prod-meta">
            <span class="text-mono text-muted" style="font-size:11px">EAN: ${_esc(d.ean)}</span>
            ${verdict ? `&nbsp;·&nbsp;<span class="badge badge-${verdict === "BUY" ? "green" : verdict === "HOLD" ? "yellow" : "red"}" style="font-size:9px">${verdict}</span>` : ""}
            ${_ek > 0 ? `&nbsp;·&nbsp;<span style="font-size:11px;color:var(--text-muted)">EK: <strong style="color:var(--text-secondary)">${_fmtEur(_ek)}</strong></span>` : ""}
          </div>
          <div class="mp-prod-stats">
            ${d.ebay?.sales_30d   != null ? `<span>📦 <strong>${d.ebay.sales_30d}</strong> Verkäufe (30 Tage)</span>` : ""}
            ${d.ebay?.days_to_cash != null ? `<span>⏱ <strong>${d.ebay.days_to_cash}d</strong> Cashflow</span>` : ""}
            ${["HOT","OK","RISK"].includes(d.kaufland?.demand_label)
              ...
              : ""}
            ${d.amazon?.asin
              ? `<a href="https://www.amazon.de/dp/${_esc(d.amazon.asin)}" target="_blank" style="font-size:10px;color:var(--text-muted);text-decoration:none" title="Amazon Produktseite">🔗 ASIN: <span class="text-mono">${_esc(d.amazon.asin)}</span></a>`
              : `<a href="https://www.amazon.de/s?k=${encodeURIComponent(d.ean)}" target="_blank" style="font-size:10px;color:var(--text-muted);text-decoration:none" title="Amazon nach EAN suchen">🔍 Auf Amazon suchen</a>`
            }
          </div>
        </div>
      </div>
  ...
  function buildCard(p) {
    ...
    if (p.id === "kaufland" && d?.kaufland) {
      ...
      extraInfo = `
        <div class="mp-card-extra">
          ...
          ${kd.min_shipping != null && kd.min_shipping > 0 ? `<span>zzgl. Versand ${_fmtEur(kd.min_shipping)}</span>` : kd.min_shipping === 0 ? `<span>Kostenloser Versand</span>` : ""}
        </div>
      `;
    }
    if (p.id === "amazon" && d?.amazon) {
      ...
      extraInfo = `
        <div class="mp-card-extra">
          ${ad.buybox_price    != null ? `<span>Buy Box: ${_fmtEur(ad.buybox_price)}</span>` : ""}
          ${ad.marketplace_new != null ? `<span>Marketplace (Neu): ${_fmtEur(ad.marketplace_new)}</span>` : ""}
          ${ad.amazon_direct   != null ? `<span>Amazon direkt: ${_fmtEur(ad.amazon_direct)}</span>` : ""}
          ${ad.asin
            ? `<a href="https://www.amazon.de/dp/${_esc(ad.asin)}" target="_blank" class="mp-asin-link">Produktseite ${_esc(ad.asin)} →</a>`
            : `<a href="https://www.amazon.de/s?k=${encodeURIComponent(d.ean)}" target="_blank" class="mp-asin-link">Auf Amazon suchen</a>`
          }
        </div>
      `;
    }
    if (p.id === "ebay" && d?.ebay) {
      ...
      extraInfo = `
        <div class="mp-card-extra">
          ${ed.listing_count  != null ? `<span>${ed.listing_count} Angebote</span>` : ""}
          ${ed.price_avg      != null && ed.price_avg !== ed.price ? `<span>Ø ${_fmtEur(ed.price_avg)}</span>` : ""}
          ${ed.price_min != null ? `<span>Ab ${_fmtEur(ed.price_min)}</span>` : ""}
        </div>
      `;
    }
    ...
    return `
      <div class="mp-card${best ? " mp-card-best" : ""}" id="mpCard_${p.id}"
        ...
        ${best ? `<div class="mp-card-best-badge">⭐ Beste Option</div>` : ""}
        ...
        <div class="mp-card-price-wrap">
          ${price != null ? `
            <div class="mp-card-price-label">${isLive ? "Live-Preis" : "Manueller Preis"}</div>
            <div class="mp-card-price-value">${_fmtEur(price)}</div>
          ` : (() => {
            ...
            if (err === "NOT_FOUND" || err === "NO_MATCH") {
              hint = `<div class="mp-unavail-hint">${p.id === "amazon" ? "Produkt nicht auf Amazon gelistet" : "Produkt nicht auf Kaufland gelistet"}</div>`;
            } else if (err && (err.includes("BLOCK") || err.includes("block") || err.includes("429") || err.includes("403"))) {
              hint = `<div class="mp-unavail-hint mp-unavail-blocked">⚠ Kaufland vorübergehend blockiert</div>`;
            } else if (err === "NO_PRICE") {
              hint = `<div class="mp-unavail-hint">Preis nicht verfügbar</div>`;
            } else if (err) {
              hint = `<div class="mp-unavail-hint">Nicht verfügbar</div>`;
            }
            return `
              ${hint}
              <div class="mp-card-price-label" style="margin-top:${hint ? "6px" : "0"}">Preis manuell eingeben</div>
  ...
  function renderMetrics(calc, p) {
    ...
    return `
      <div class="mp-metric">
        ...
        <span class="mp-metric-v" style="color:${profitColor}">${_fmtEur(profit)}</span>
      </div>
      <div class="mp-metric">
        <span class="mp-metric-l">Marge</span>
      ...
      <div class="mp-metric">
        ...
        <span class="mp-metric-v text-muted">−${_fmtEur(fee)}</span>
      </div>
    `;
  }

  function renderMetricsEmpty(noPrice) {
    return `<div class="mp-metrics-empty">${noPrice ? "Preis eingeben, um Gewinn zu berechnen" : "—"}</div>`;
  }
  ...
  function updateCardCalc(p) {
    ...
    PLATFORMS.filter(pl => _selectedPlatforms.has(pl.id)).forEach(pl => {
      ...
      if (best && !badge) {
        ...
        badge.textContent = "⭐ Beste Option";
        card.prepend(badge);
      } else if (!best && badge) {
        badge.remove();
      }
  ...
  function renderTable() {
    ...
    tableEl.innerHTML = `
      <div class="mp-table-title">Direktvergleich</div>
          ...
          ${rows.map(r => {
            ...
            return `
              <tr class="${isBest ? "mp-table-best" : ""}">
                ...
                <td>${r.price != null ? _fmtEur(r.price) : `<span class="text-muted">—</span>`}</td>
                <td class="text-muted">${r.fee != null ? `−${_fmtEur(r.fee)}` : "—"}</td>
                <td style="font-weight:600;color:${r.profit != null ? (r.profit >= 7 ? "var(--green)" : r.profit >= 0 ? "var(--yellow)" : "var(--red)") : "var(--text-muted)"}">
                  ${r.profit != null ? _fmtEur(r.profit) : "—"}
                </td>
  ...
  // ── Utility (local to IIFE to avoid conflicts with global esc/fmtEur) ───
  function _esc(s) {
