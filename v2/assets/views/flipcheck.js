...
const FlipcheckView = (() => {
  ...
  const CATEGORIES = [
    ...
    { id: "computer_tablets",  label: "Computer, Tablets & Netzwerk",  group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "drucker",           label: "Drucker",                        group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "foto_camcorder",    label: "Foto & Camcorder",               group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "handys",            label: "Handys & Kommunikation",         group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "haushaltsgeraete",  label: "Haushaltsgeräte",                group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "konsolen",          label: "Konsolen / Videospiele",         group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "scanner",           label: "Scanner",                        group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "speicherkarten",    label: "Speicherkarten",                 group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "tv_video_audio",    label: "TV, Video & Audio",              group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "koerperpflege",     label: "Elektr. Körperpflege & Styling", group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    // ── Zubehör: 11 % bis €990, danach 3 % ──────────────────────────────
    { id: "drucker_zubehoer",  label: "Drucker- & Scanner-Zubehör",    group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "handy_zubehoer",    label: "Handy-Zubehör",                  group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "batterien",         label: "Haushaltsbatterien & Strom",     group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "kabel",             label: "Kabel & Steckverbinder",         group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "kameras_zubehoer",  label: "Kameras, Drohnen & Fotozubehör",group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "notebook_zubehoer", label: "Notebook- & Desktop-Zubehör",   group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "objektive",         label: "Objektive & Filter",             group: "Geräte (7%)",  tiers: [[null, 0.07]] },
    { id: "stative",           label: "Stative & Zubehör",             group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "tablet_zubehoer",   label: "Tablet & eBook Zubehör",        group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "tastaturen_maeuse", label: "Tastaturen, Mäuse & Pointing",   group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "tv_zubehoer",       label: "TV- & Heim-Audio-Zubehör",      group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "pc_zubehoer",       label: "PC & Videospiele Zubehör",      group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    { id: "audio_zubehoer",    label: "Zubehör Audiogeräte",           group: "Zubehör (12%)",  tiers: [[null, 0.12]] },
    // ── Sonstige Flat-Rate ───────────────────────────────────────────────
    { id: "mode",              label: "Mode / Bekleidung",              group: "Sonstiges (Flat)",     tiers: [[990, 0.12], [null, 0.03]]  },
    { id: "sport_freizeit",    label: "Sport & Freizeit",               group: "Sonstiges (Flat)",     tiers: [[null, 0.14]] },
    { id: "spielzeug",         label: "Spielzeug / LEGO",               group: "Sonstiges (Flat)",     tiers: [[990, 0.12], [null, 0.03]] },
    { id: "haushalt",          label: "Haushalt",                       group: "Sonstiges (Flat)",     tiers: [[null, 0.14]] },
    { id: "garten",            label: "Garten",                         group: "Sonstiges (Flat)",     tiers: [[null, 0.13]] },
    { id: "buecher",           label: "Bücher & Medien",                group: "Sonstiges (Flat)",        tiers: [[990, 0.12], [null, 0.03]]  },
    { id: "sonstiges",         label: "Sonstiges",                      group: "Sonstiges (Flat)",        tiers: [[null, 0.14]]  },
  ...
  const AMZ_REFERRAL_FEES = {
    ...
    handmade: 0.12, nahrungsergaenzung: 0.08,
    bildende_kunst: 0.20,
    sonstiges: 0.15,
  };
  ...
  function _renderSneakerTable(sizes, platform) {
    ...
    for (const s of sizes) {
      ...
      html += `<tr style="border-bottom:1px solid var(--border1);background:${rowBg}">
        ...
        <td style="text-align:right;padding:5px 10px;font-weight:700;color:${profitColor}">${s.profit != null ? profitSign + s.profit.toFixed(0) + "€" : "—"}</td>
        <td style="text-align:right;padding:5px 10px;color:var(--text-muted)">${s.sales_30d || 0}</td>
      </tr>`;
    }
  ...
  function renderResultSneakerSingle(data, sku, ek, platform) {
    ...
    if (platform === "stockx") {
      chipsHtml = `
        ${pData.sales_90d != null ? `<div class="fc-market-chip"><span class="fc-market-chip-l">90d Sales</span><span class="fc-market-chip-v">${pData.sales_90d}</span></div>` : ""}
        ${pData.sales_72h != null ? `<div class="fc-market-chip"><span class="fc-market-chip-l">72h Sales</span><span class="fc-market-chip-v">${pData.sales_72h}</span></div>` : ""}`;
    }
  ...
  function _initSneakerChart(container, chart) {
    if (!chart?.length || chart.length < 3) return;
    const canvas = container.querySelector("#sneakerChart");
    if (!canvas || typeof Chart === "undefined") return;

    // Aggregate by date (average price per day)
    const byDate = {};
    for (const pt of chart) {
      if (!pt.date || !pt.price) continue;
